...
namespace CDS.GUI {
	...
	public abstract class GuiPoint : GuiBinded, ISelectable {
		...
		protected const int POINT_NORMAL_SIZE = 2;
		...
		public override void Paint( Graphics g, Rectangle r, int offx, int offy ) {
			if( selected ) {
				g.FillRectangle( Brushes.Red, x + r.X - offx-POINT_SELECTED_SIZE/2, y + r.Y - offy-POINT_SELECTED_SIZE/2, POINT_SELECTED_SIZE, POINT_SELECTED_SIZE );
