...
namespace CDS.GUI {
	...
	public abstract class MouseAgent {
		...
		public abstract void StartDrag( CS_Element elem );
	...
	public class StaticViewMouseAgent : MouseAgent {
		...
		public override void StartDrag( CS_Element elem ) {

			UmlClass si;

			if( parent.proj.items.ContainsKey( elem.qualifiedName ) )
				si = parent.proj.items[elem.qualifiedName] as UmlClass;
			else {
				si = (UmlClass)elem.ToUML();
				parent.proj.items[si.fullname] = si;
			}
			dropitem = GuiClass.fromUML( si );
			dropitem.parent = parent;
			parent.RefreshObject( dropitem );
		...
		private void NewRelation( GuiClass c1, GuiClass c2, GuiConnectionType t ) {
			GuiConnection c = new GuiConnection( new GuiConnectionPoint( c1, 1, .5f, 0 ), new GuiConnectionPoint( c2, 3, .5f, 1 ), t, parent );
		...
		public override void Drop( ) {
			...
			dropitem.id = parent.RegisterItemID( dropitem.st.fullname, dropitem );
			dropitem.Invalidate();
			parent.Undo.Push( new CreateOperation( dropitem ), false );
			action = MouseAction.None;
		...
		public override void Drag( int x, int y ) {
			...
			if( dropitem != null )
				dropitem.Moving(x - GuiItem.inflate,y - GuiItem.inflate,ref ux,ref uy);
		}
		...
		public override void MouseDown( int x, int y, MouseButtons b, Keys modif, int realx, int realy ) {
			...
			if( b == MouseButtons.Left ) {
				...
				if( (modif & Keys.Control) == Keys.Control || current_operation == (int)MouseOp.DrawConnection ) {
					...
					conn = new GuiConnection( new GuiConnectionPoint( conn_item, ux, uy, 0 ), new GuiConnectionPoint( x, y, 1 ), GuiConnectionType.Association, parent );
		...
		public override void MouseUp(MouseButtons b) {
			...
			parent.proj.container.SetDefaultDrawingMode();
			original_selected = null;
				...
				case MouseAction.Drag:
					if( dropitem != null && dropitem.place.IntersectsWith(pagepiece) ) {
						...
						using( Pen p = new Pen( Color.White ) ) {
							...
							g.DrawRectangle( p, x + GuiItem.inflate, y + GuiItem.inflate, dropitem.place.Width - 2*GuiItem.inflate - 1, dropitem.place.Height - 2*GuiItem.inflate - 1 );
						}
					}
