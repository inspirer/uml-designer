...
namespace UMLDes.GUI
{
	...
	public class Geometry {
		...
		public static bool intersect_segment_and_segment( int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, out int x, out int y ) {
		...
		public static bool rectangle_intersects_with_line( Rectangle r, int x1, int y1, int x2, int y2 ) {
			...
			if( intersect_segment_and_segment( r.Left, r.Top, r.Right, r.Top, x1, y1, x2, y2, out _x, out _y )
				|| intersect_segment_and_segment( r.Left, r.Top, r.Left, r.Bottom, x1, y1, x2, y2, out _x, out _y )
				|| intersect_segment_and_segment( r.Left, r.Bottom, r.Right, r.Bottom, x1, y1, x2, y2, out _x, out _y )
				|| intersect_segment_and_segment( r.Right, r.Top, r.Right, r.Bottom, x1, y1, x2, y2, out _x, out _y ) )
				return true;
		...
		public static void point_triangle_on_segment( out Point[] pol, int x1, int y1, int x2, int y2, int delta, int delta2 ) {
			...
			if( div > 0 ) {
				...
				pol[1].X = x1; pol[1].Y = y1;
				pol[0].X = nx1 + dx; pol[0].Y = ny1 + dy;
				pol[2].X = nx1 - dx; pol[2].Y = ny1 - dy;
			} else {
				pol[1].X = x1; pol[1].Y = y1;
				pol[0].X = x1-delta2; pol[0].Y = y1+delta;
